# 爬取添加内容，主播的开播详情，返回错误信息提示，进行数据分析，发弹幕人的等级分析,词云，弹幕发的最多的
import socket
import pymongo
import re
import time
        ...
        if not data_pac:
            ...
            for i in range(0, len(danmu)):
                try:
                    ...
                    # 避免在数据库中插入重复的内容
                    if not col_msg.find_one({'danmu': YT['danmu'], 'sender_uid': YT['sender_uid']}):
                        col_msg.insert(YT)
                        print(YT)
                        print('成功保存到MongoDB->Douyu_danmu')
                    if col_msg.find().count() >= 5000:
                        print('已成功存储5000条弹幕！')
                        flag = False
                        print('生成词云中......')
                        draw_wordcloud()
...
if __name__ == '__main__':
    ...
    p1 = multiprocessing.Process(target=get_run, args=(room_id,))
